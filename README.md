...
[![Version](https://img.shields.io/badge/version-0.10.4-blue.svg)](https://github.com/mgd34msu/goodvibes-homeassistant)
...
| Voice | The conversation entity works as the agent behind a [Wyoming](https://www.home-assistant.io/integrations/wyoming/) satellite — wake word, STT, and TTS stay with Home Assistant/Wyoming | [voice-assist.md](docs/voice-assist.md) |
| GoodVibes Home panel | Admin-only sidebar UI for Home Graph status, sync, source/node/edge/issue browsing, the visual map, URL/note/file ingest, source-backed questions, link/unlink, review, reindex, generated pages, export/import, reset, and packets — talks to Home Assistant, never directly to the daemon | [home-graph.md](docs/home-graph.md) |
...
| Sensors, repairs, update | Diagnostic sensors for daemon status, last reply, active session/message/agent IDs, tool catalog and Home Graph status/issues/sources; repairs for Home Graph availability and unresolved issues; a GitHub-release-backed update entity | — |
...
This integration targets the **latest** published `@pellux/goodvibes-sdk`, always — it is a thin client over stable daemon HTTP routes, not a build pinned to one SDK release, so there is no per-release version to chase. The single moving label is the newest npm version the daemon contract was last validated against, tracked in `const.SDK_VALIDATED_VERSION` and echoed against the live npm version in CI as an informational nudge that never fails the build.
