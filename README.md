...
Custom Home Assistant integration for the GoodVibes daemon Home Assistant surface from `@pellux/goodvibes-sdk` `0.28.10`.
...
- GoodVibes daemon using `@pellux/goodvibes-sdk@0.28.10` or newer.
...
The integration supports the SDK `0.28.10` Home Graph daemon routes:
...
Do not manually delete SDK database rows or import over the current Home Graph space to recover from bad historical ingest/link/page data. Export/import are for backup and transfer, not reset. Use the SDK-owned admin reset route for the target `homeassistant:<installationId>` space only, then sync the Home Assistant snapshot, reingest or relink manuals and uploads, run reindex/refinement/page generation, and retest Ask, Pages, and Map from the clean space. The panel and `goodvibes.home_graph_reset` service require typed `RESET` confirmation before calling the dangerous daemon route.
...
Ask responses are rendered directly from SDK fields. The panel shows the synthesized answer text, confidence/mode, extracted facts, gaps, linked sources, and linked Home Assistant objects. The integration does not parse PDFs, rank snippets, or synthesize answers locally. SDK `0.28.10` also supports `knowledgeSpaceId: "homeassistant"` as a namespace alias for base knowledge Ask calls.
