...
Custom Home Assistant integration for the GoodVibes daemon Home Assistant surface from `@pellux/goodvibes-sdk` `0.27.5`.
...
- GoodVibes daemon using `@pellux/goodvibes-sdk@0.27.5` or newer.
...
The integration supports the SDK `0.27.5` Home Graph daemon routes:
...
The snapshot sent by `goodvibes.sync_home_graph` includes entities, devices, areas, automations, scripts, scenes, labels where available, integrations, helper metadata, selected current state attributes, integration documentation/source candidates, source registry metadata, and `pageAutomation` with device passports and room pages enabled. SDK `0.27.5` provides semantic Home Graph answers synthesized by the daemon, returns `answer.text`, `sources`, `linkedObjects`, `facts`, `gaps`, and `synthesized`, anchors object-specific questions to matching Home Assistant graph nodes, uses strict semantic candidates after object-scoped search so unrelated manuals do not win on generic feature/spec terms, filters deterministic facts by query intent, bounds provider-backed semantic calls with SDK timeout/abort/concurrency controls, lazily repairs relevant linked manuals before ranking, rejects garbled/raw-PDF extraction text as repair-needed answer material, reparses existing uploaded PDFs without reupload, semantically enriches existing sources, repairs weak or binary extracted text, auto-links manuals to Home Assistant nodes by model/entity/device identity, regenerates generated pages with source-backed semantic facts after repair, accepts Home Assistant-style snake_case fields, preserves durable Home Graph review decisions across sync/refresh, returns generated pages through `/api/homeassistant/home-graph/pages`, and returns a visual graph map with SDK-owned filters and Home Assistant facets through `/api/homeassistant/home-graph/map`.
