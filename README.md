...
Custom Home Assistant integration for the GoodVibes daemon Home Assistant surface from `@pellux/goodvibes-sdk` `0.33.24`.
...
- GoodVibes daemon using `@pellux/goodvibes-sdk@0.33.24` or newer.
...
Releases are published from tags in the form `v0.5.68`. The release workflow uploads `goodvibes.zip`, which contains `custom_components/goodvibes/` for Home Assistant and HACS installs. After an update is installed, restart Home Assistant so the new Python and frontend files are loaded.
...
The integration supports the SDK `0.33.24` Home Graph daemon routes:
...
Ask responses are rendered directly from SDK fields. The panel shows the synthesized answer text, confidence/mode, repair/refinement metadata, extracted facts, gaps, linked sources, and linked Home Assistant objects. Fact cards preserve daemon-provided linkage fields such as `subject`, `subjectIds`, `linkedObjectIds`, and `targetHints` when present. The integration does not parse PDFs, rank snippets, or synthesize answers locally. SDK `0.33.24` also supports `knowledgeSpaceId: "homeassistant"` as a namespace alias for base knowledge Ask calls.
