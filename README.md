...
Custom Home Assistant integration for the GoodVibes daemon Home Assistant surface from `@pellux/goodvibes-sdk` `0.28.20`.
...
- GoodVibes daemon using `@pellux/goodvibes-sdk@0.28.20` or newer.
...
The integration supports the SDK `0.28.20` Home Graph daemon routes:
...
The pages response includes `ok`, `spaceId`, and `pages`. Each page has a daemon source record plus optional artifact metadata and markdown content when `include_markdown` is true. SDK `0.28.20` filters `knowledge.answer_gap`, `knowledge.semantic_gap`, and `knowledge.intrinsic_gap` records out of generated page issue sections so generated pages show actionable Home Graph issues instead of internal knowledge gaps. The GoodVibes Home Pages tab renders these records as a wiki-style page browser and reader; direct regeneration, export/import, and reset controls are kept in collapsed maintenance sections. The reader uses SDK page subject, target, neighbor, and related-page metadata when present, and also derives fallback internal navigation from returned page metadata and markdown rows: matching generated pages open directly, while missing page targets, model names, areas, integrations, entities, and device IDs can jump into scoped map/search views.
...
Ask responses are rendered directly from SDK fields. The panel shows the synthesized answer text, confidence/mode, extracted facts, gaps, linked sources, and linked Home Assistant objects. Fact cards preserve daemon-provided linkage fields such as `subject`, `subjectIds`, `linkedObjectIds`, and `targetHints` when present. The integration does not parse PDFs, rank snippets, or synthesize answers locally. SDK `0.28.20` also supports `knowledgeSpaceId: "homeassistant"` as a namespace alias for base knowledge Ask calls.
