...
| Home Assistant services | The full service catalog: prompting and daemon control, the Home Graph service set (sync, ingest, link, ask, review, map, export/import/reset), causal attribution, and habit-mining review. Each service and what it does is tabulated in full | [services.md](docs/services.md) |
| Mail and calendar | A calendar entity backed by the daemon's calendar (events in the calendar UI and usable in automations, including event creation), plus a matching set of mail and calendar services, tabulated in full at the link. The daemon owns the accounts and every credential; nothing is stored here, so setup done on any surface works on all of them | [mail-calendar.md](docs/mail-calendar.md) |
