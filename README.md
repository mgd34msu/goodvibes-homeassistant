...
The integration registers an admin-only Home Assistant sidebar panel named `GoodVibes Home`. It follows the same custom-panel pattern used by HACS: the panel is registered as Home Assistant's `custom` panel with `_panel_custom` config, and the sidebar uses the local `goodvibes:home` iconset generated from `custom_components/goodvibes/frontend/gv-icon-sidebar.svg`. The integration brand icon lives at `custom_components/goodvibes/brand/icon.png`.
