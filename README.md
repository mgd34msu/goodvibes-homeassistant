...
The integration registers an admin-only Home Assistant sidebar panel named `GoodVibes Home`. Home Assistant custom panels load their page code through `panel_custom`; the sidebar uses the local `goodvibes:home` iconset generated from `custom_components/goodvibes/frontend/gv-icon-sidebar.svg`, while the integration brand icon lives at `custom_components/goodvibes/brand/icon.png`.
