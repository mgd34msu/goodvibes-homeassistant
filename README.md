...
Custom Home Assistant integration for the GoodVibes daemon Home Assistant surface from `@pellux/goodvibes-sdk` `0.28.9`.
...
- GoodVibes daemon using `@pellux/goodvibes-sdk@0.28.9` or newer.
...
The integration supports the SDK `0.28.9` Home Graph daemon routes:
...
Do not manually delete SDK database rows or import over the current Home Graph space to recover from bad historical ingest/link/page data. Export/import are for backup and transfer, not reset. Use the SDK-owned Home Graph reset route once it is published, then sync the Home Assistant snapshot, reingest or relink manuals and uploads, run reindex/refinement/page generation, and retest Ask, Pages, and Map from the clean `homeassistant:<installationId>` space.
...
Ask responses are rendered directly from SDK fields. The panel shows the synthesized answer text, confidence/mode, extracted facts, gaps, linked sources, and linked Home Assistant objects. The integration does not parse PDFs, rank snippets, or synthesize answers locally. SDK `0.28.9` also supports `knowledgeSpaceId: "homeassistant"` as a namespace alias for base knowledge Ask calls.
