...
Custom Home Assistant integration for the GoodVibes daemon Home Assistant surface from `@pellux/goodvibes-sdk` `0.28.16`.
...
- GoodVibes daemon using `@pellux/goodvibes-sdk@0.28.16` or newer.
...
The integration supports the SDK `0.28.16` Home Graph daemon routes:
...
Ask responses are rendered directly from SDK fields. The panel shows the synthesized answer text, confidence/mode, extracted facts, gaps, linked sources, and linked Home Assistant objects. The integration does not parse PDFs, rank snippets, or synthesize answers locally. SDK `0.28.16` also supports `knowledgeSpaceId: "homeassistant"` as a namespace alias for base knowledge Ask calls.
