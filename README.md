...
Custom Home Assistant integration for the GoodVibes daemon Home Assistant surface from `@pellux/goodvibes-sdk` `0.33.30`.
...
- GoodVibes daemon using `@pellux/goodvibes-sdk@0.33.30` or newer.
...
The Home Assistant long-lived access token belongs in the daemon config as `surfaces.homeassistant.accessToken`. The integration stores the daemon URL, daemon bearer token, webhook secret, event type, and Home Graph settings in the Home Assistant config entry. The setup UI masks the daemon token and webhook secret fields.

### Credential Handling

This integration does not store daemon credentials in `/tmp`. Browser uploads use a temporary file only for the uploaded document/photo bytes, then unlink it after the daemon ingest call finishes.

GoodVibes `goodvibes://...` secret references are daemon-side config values. They work for daemon-owned secrets such as `surfaces.homeassistant.accessToken` and `surfaces.homeassistant.webhookSecret`, but they cannot authenticate the first HTTP request from Home Assistant to that same daemon. The integration needs a daemon bearer token or session token that Home Assistant can actually send in the `Authorization` header.

The current GoodVibes-native bearer-token path is the companion/operator token generated by the TUI/daemon pairing flow. GoodVibes stores the canonical token under the daemon home, usually `~/.goodvibes/daemon/operator-tokens.json`, with owner-only permissions. Home Assistant still stores the token value it receives, because it must attach that value to daemon API requests.

Preferred future shape:

- Home Assistant starts a Home-Assistant-specific GoodVibes pairing flow from the config flow.
- The daemon operator approves the pairing in GoodVibes.
- The daemon issues a revocable, scoped Home Assistant integration credential.
- Home Assistant stores only that scoped credential and can rotate/revoke it without using the shared operator token.

Do not put device-specific daemon URLs or tokens in the integration source code. Configure them through the Home Assistant config entry.
...
Releases are published from tags in the form `v0.5.70`. The release workflow uploads `goodvibes.zip`, which contains `custom_components/goodvibes/` for Home Assistant and HACS installs. After an update is installed, restart Home Assistant so the new Python and frontend files are loaded.
...
The integration supports the SDK `0.33.30` Home Graph daemon routes:
...
Ask responses are rendered directly from SDK fields. The panel shows the synthesized answer text, confidence/mode, repair/refinement metadata, extracted facts, gaps, linked sources, and linked Home Assistant objects. Fact cards preserve daemon-provided linkage fields such as `subject`, `subjectIds`, `linkedObjectIds`, and `targetHints` when present. The integration does not parse PDFs, rank snippets, or synthesize answers locally. SDK `0.33.30` also supports `knowledgeSpaceId: "homeassistant"` as a namespace alias for base knowledge Ask calls.
