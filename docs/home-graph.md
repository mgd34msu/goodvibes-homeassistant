...
The integration targets these SDK `0.34.0` Home Graph routes:
...
Daemon artifact size is controlled by `storage.artifacts.maxBytes`; SDK `0.34.0` defaults to `512 MiB`. Home Assistant and reverse proxies in front of it may need matching upload size and timeout settings for large browser uploads.
...
SDK `0.34.0` also supports `knowledgeSpaceId: "homeassistant"` as a namespace alias for base knowledge Ask calls.
...
SDK `0.34.0` may also return `semantic.selfImprovement`, refinement task IDs, `truncated`, and `budgetExhausted`. Broad repair work may be queued or coalesced for asynchronous refinement instead of completed inside the reindex request.
